...
// GIẢ ĐỊNH: Import các hàm API cần thiết
...
const [tourStep, setTourStep] = useState(1);
  ...
  const [isFinishModalOpen, setIsFinishModalOpen] = useState(false);
    const [isFinishing, setIsFinishing] = useState(false); // Đã thêm: cho Modal loading
...
// VẤN ĐỀ 2: NÚT GỢI Ý
const handleChatGPT = (type) => {
    let suggestionText = "";
    if (type === 'unknown') {
        suggestionText = "Tôi xin lỗi, hiện tại tôi chưa có đủ kiến thức hoặc kinh nghiệm để trả lời câu hỏi này. Tôi sẽ tìm hiểu thêm sau buổi phỏng vấn.";
    } else if (type === 'no-expertise') {
        suggestionText = "Tôi đã từng làm việc trong lĩnh vực liên quan, nhưng không có kinh nghiệm chuyên sâu về chủ đề này.";
    } else {
        toast.info("Chức năng ChatGPT đang phát triển!");
        return;
    }
    // Thêm câu trả lời vào ô text
    setAnswer(suggestionText);
};

const handleNextQuestion = async () => {
  const isLastQuestion = questionIndex === questions.length - 1;

  // VẤN ĐỀ 1: KHI BẤM "QUA CÂU TIẾP THEO" Ở CÂU CUỐI, CẦN BẬT LOADING VÀ GỌI HANDLE FINISH
  if (isLastQuestion) {
    return handleFinish(true); // Gửi câu trả lời cuối và kết thúc
  }
...
const handleFinish = async (isForcedFinish = false) => {
  ...
  // 2. BẬT LOADING TOÀN PHIÊN (VẤN ĐỀ 1)
  if (isForcedFinish) setIsFinishing(true); 
    
    // Sử dụng toast loading cho quá trình hoàn thành (nếu không phải là từ handleNextQuestion)
    let toastFinishId = isForcedFinish ? toast.loading("Đang hoàn tất phiên phỏng vấn...") : null;

  // 3. Gửi câu trả lời CUỐI CÙNG (Giữ nguyên)
  if (sessionId && questions.length > 0) {
    const current = questions[questionIndex];
    const payload = {
      sessionId: parseInt(sessionId),
  ...
  // --- VẤN ĐỀ 3: LOGIC TRỪ LƯỢT DÙNG THỬ ---
  ...
  if (userStatus === 0) { // CHỈ TRỪ KHI LÀ TÀI KHOẢN MIỄN PHÍ
    try {
      const response = await decrementTrialApi(); 
    
    // FIX VẤN ĐỀ 3: CẬP NHẬT THỦ CÔNG TRIAL COUNT SAU KHI API THÀNH CÔNG
    ...
    console.log("Đã gọi API trừ đi 1 lần dùng thử thành công và cập nhật session.");
  ...
  if (isLastQuestion) {
    ...
    try {
      ...
      toast.update(toastFinishId, { render: "Hoàn thành! Đang tạo báo cáo...", type: "success", isLoading: false, autoClose: 3000 });
      navigate(`/ReportInterview?sessionId=${sessionId}`);
    } catch (error) {
      toast.update(toastFinishId, { render: "Lỗi hoàn thành phiên.", type: "error", isLoading: false, autoClose: 3000 });
      navigate("/");
    }
  } else {
    // ... (Logic kết thúc sớm)
    try {
      ...
      toast.update(toastFinishId, { render: "Phiên phỏng vấn đã bị hủy.", type: "warning", isLoading: false, autoClose: 3000 });
      navigate("/");
    } catch (error) {
      toast.update(toastFinishId, { render: "Lỗi khi hủy phiên.", type: "error", isLoading: false, autoClose: 3000 });
      navigate("/");
    } finally {
...
return (
 ...
 <div className="interview-main">
  ...
  {/* VẤN ĐỀ 2: NÚT GỢI Ý */}
  <button className="btn-secondary" onClick={() => handleChatGPT('unknown')}>
   Tôi không biết
  </button>
  <button className="btn-secondary" onClick={() => handleChatGPT('no-expertise')}>
   Không có chuyên môn
  </button>
  ...
  <button 
      ...
      >
        {isLoading || (questionIndex === questions.length - 1 && isFinishing) ? (
  ...
  <button className="btn-finish" onClick={() => handleFinish(false)} disabled={isLoading || isFinishing}>
   Hoàn thành
  </button>
  </div>
 </div>
